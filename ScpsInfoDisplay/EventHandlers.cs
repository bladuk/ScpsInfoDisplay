...
namespace ScpsInfoDisplay
{
    public class EventHandlers
    {
        ...
        public void OnReloadedConfigs() => ScpsInfoDisplay.Singleton.ValidateConfigs();
            ...
            for (; ; )
            {
                ...
                try
                {
                    if (player != null)
                    {
                        ...
                        {
                            text = Player.List.Where(p => p?.SessionVariables.ContainsKey(integration.Key) == true).Aggregate(text, (current, any) => current + $"<align={ScpsInfoDisplay.Singleton.Config.TextAlignment}>" + (player == any && ScpsInfoDisplay.Singleton.Config.MarkPlayerInList ? ScpsInfoDisplay.Singleton.Config.PlayersMarker + " " : "") + ProcessStringVariables(integration.Value, player, any) + "</align>\n");
                        }

                        foreach (var scp in Player.List.Where(p => p?.Role.Team == Team.SCP && ScpsInfoDisplay.Singleton.Config.DisplayStrings.ContainsKey(p.Role.Type)))
                        {
